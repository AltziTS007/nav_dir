...
DynamixelController::DynamixelController()
  :node_handle_(""),
   ...
   use_moveit_(false),
   wheel_separation_(0.0f), //wheel_distance_from_center_
   wheel_radius_(0.0f),
   is_moving_(false)
{
...
// systhma kinhshw apo auto to video https://www.youtube.com/watch?v=wwQQnSWqB7A&t=327s
...
//       = r * ((RPM * Goal_Velocity) * 0.10472)		=> Goal_Velocity = V / (r * RPM * 0.10472) = V * VELOCITY_CONSTATNE_VALUE
 ...
 cout<<"Left speed:"<< wheel_velocity[LEFT]<<"           "<<"right velocity:"<<wheel_velocity[RIGHT]<<"         "<<"Back velocity:"<<wheel_velocity[BACK]<<endl;
  ...
  //wheel_velocity[LEFT]  = (-robot_linX_vel*0.33) -(robot_linY_vel*0.57 )+ (0.33 *robot_ang_vel); 
  //wheel_velocity[RIGHT] = (-robot_linX_vel*0.33)  +(robot_linY_vel*0.57) +( 0.33 * robot_ang_vel);
  //wheel_velocity[BACK]  =  (robot_linX_vel*0.66) + (0.33* robot_ang_vel);
   ...
   if (strcmp(dxl_wb_->getModelName(id_array[0]), "XL-320") == 0)
    {
     if (wheel_velocity[LEFT] == 0.0f) dynamixel_velocity[LEFT] = 0;
      else if (wheel_velocity[LEFT] < 0.0f) dynamixel_velocity[LEFT] = ((-1.0f) * wheel_velocity[LEFT]) * velocity_constant_value  ;
      ...
      else if (wheel_velocity[RIGHT] < 0.0f)  dynamixel_velocity[RIGHT] = ((-1.0f) * wheel_velocity[RIGHT]) * velocity_constant_value ;
      ...
      else if (wheel_velocity[BACK] < 0.0f)  dynamixel_velocity[BACK] = ((-1.0f) * wheel_velocity[BACK]) * velocity_constant_value;
...
cout<<"Left velocity:"<< dynamixel_velocity[LEFT]<<"           "<<"right velocity:"<<dynamixel_velocity[RIGHT]<<"         "<<"Back velocity:"<<dynamixel_velocity[BACK]<<endl<<endl;
   }
  }
  else if (dxl_wb_->getProtocolVersion() == 1.0f)
  {
